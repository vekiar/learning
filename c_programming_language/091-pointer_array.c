...
int main()
{
  ...
  if ((nlines = readlines(lineptr, MAXLINES)) >= 0) {     // if your input is >= 0 lines
...
int readlines(char *lineptr[], int maxlines)              // read input lines
{
  ...
  while ((len = getlines(line, MAXLEN)) > 0)              // as long as the line as a length > 0
    if (nlines >= maxlines || (p = alloc(len)) == NULL)   // if the number of lines exceeds maxlines, or allocating <len> in the pointer array is not possible
